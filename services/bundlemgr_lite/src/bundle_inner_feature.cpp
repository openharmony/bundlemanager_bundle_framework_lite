...
uint8_t BundleInnerFeature::GetSvcIdentityInfo(SvcIdentityInfo *info, const SvcIdentity *svc, const char *reqPath,
    IpcIo *req)
{
    if (info == nullptr) {
        HILOG_ERROR(HILOG_MODULE_APP, "BundleMS inner feature malloc service info info failed");
...
{
    if ((req == nullptr) || (reply == nullptr)) {
        HILOG_ERROR(HILOG_MODULE_APP, "BundleMS UninstallInnerBundle, request or reply is null");
        return ERR_APPEXECFWK_OBJECT_NULL;
    }
    size_t length = 0;
